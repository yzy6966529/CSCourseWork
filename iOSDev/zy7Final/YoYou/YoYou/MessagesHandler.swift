...
class MessagesHandler{
    ...
    func sendMediaMessage(senderID: String, senderName: String, url: String){
        ...
        DBProvider.Instance.MideaMessageRef.childByAutoId().setValue(data)
    }
    ...
    func sendMedia(image: Data?, video: URL?, senderID: String, senderName: String) {
        if image != nil{
            ...
            DBProvider.Instance.videoStorageRef.child(senderID + "\(NSUUID().uuidString)").putFile(from: video!, metadata: nil) {
                ...
                if err != nil {
                    // infrom the user that uploading the video has failed using delegation
    ...
    func observeMediaMessages() {
        DBProvider.Instance.MideaMessageRef.observe(DataEventType.childAdded) {
            (snapshot: DataSnapshot) in
