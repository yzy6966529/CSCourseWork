...
class SigninVC: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        let  myUser: NSManagedObject = NSEntityDescription.insertNewObject(forEntityName: "User", into: context)
        myUser.setValue("test@123.com", forKey: "userName")
        myUser.setValue("testtest", forKey: "password")
        do {
            try context.save()
        } catch  {
            print(error.localizedDescription)
        }
        emailTF.text = myUser.value(forKey: "userName") as? String
        passwordTF.text = myUser.value(forKey: "password") as? String
        
        let myDefault = UserDefaults.standard
        userWords.text = myDefault.string(forKey: "words_preference")
